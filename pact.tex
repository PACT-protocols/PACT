...
De-anonymization is possible and is a risk to volunteers who would prefer to remain anonymous.  Preventing proximity based identification of this sort is not possible to avoid in any protocol, even in manual contract tracing as done by public health services, simply because the exposure alert may contain information that is correlated with identifying information.
...
\noindent {\bf Protocol description.} We first describe a variant of the protocol {\em without} entry validation, and discuss how validation is simply handled on top if it below. Further, we assume a relevant infection window (typically, two weeks). In particular, we fix an understood time unit $\dt$, and define $\Delta$ such that $\Delta \cdot \dt$ equals the infection window.
