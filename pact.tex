...
\noindent {\bf Protocol description.} In this description, we assume that there exists a relevant infection window, i.e., upon testing positive, contacts within the last $\Delta$ units of time may have been infected. In particular, there is some understood time unit $\dt$. We typically want $\Delta \cdot \dt$ to be two weeks. 
...
\noindent {\bf Privacy/security properties.} Let us refer to users as either ``positive'' or ``negative'' depending on whether they reported positive or not. All of the following statements assume cryptographic security of $f$. Then, the above protocol has the following properties:
\begin{itemize}
